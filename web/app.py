...
def handle_unhandled_exception(error):
    rid = getattr(g, 'request_id', '-')
    # HTTPExceptions carry a deliberate status and description (404, 405, the
    # 400 from CSRFProtect, ...). Flask routes them here when no handler is
    # registered for their specific code, so without this branch every one of
    # them is reported to the caller as a 500 and the real cause is lost.
