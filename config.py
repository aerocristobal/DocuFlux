...
class Settings(BaseSettings):
    ...
    # "owner" scopes GET /api/captures to the identities the caller presents — the
    # extension's X-Client-ID, the UI session, or both, in which case it sees the union.
    # "global" restores the pre-scoping behaviour of returning every capture to any
    # caller — only appropriate on a single-user self-host where the capture list is not
    # sensitive. The full index is always available on /api/v1/admin/captures.
